...
```
Equipe | Bateria | Temp(°C) | Pressão  | CO₂(ppm) | Acelerômetro XYZ              | Status
...
```
✓ Criogênico   — gelo seco → -33.33 °C          → nenhum dano
✓ Térmico      — secador   → +50.65 °C          → nenhum dano
✓ Vácuo        — 946 mbar → 6 mbar (120s)        → aprovado (empresa BRF)
...
✓ Voo          — balão atmosférico ~40km altitude → satélite ativo durante todo o voo
```
...
Cliente (WhatsApp)
        │
        ▼
  360Dialog API           ← ponte oficial Meta/WhatsApp
  [100–300 ms]
        │
        ▼
  FastAPI Server          ← webhook receiver + orquestrador
  (Python / Render)
        │
        ▼
  OpenAI GPT              ← LLM com prompt mestre + histórico
  [1–2 segundos]
        │
        ▼
  Resposta ao cliente     ← latência total: < 3 segundos
```
...
Razão da escolha:
  ...
  ✗ Trade-off: sensível a variações de frequência e frágil a vibrações
               → mitigado com isolamento estrutural

Alternativas avaliadas:
  Dipolo  → omnidirecional, ganho médio, simples — descartado pelo ganho
  Patch   → baixo perfil, leve, fácil fabrico    — descartado pela banda estreita
  Monopolo→ design simples, omnidirecional       — descartado pelo baixo ganho
```
...
🥉  Bronze Olimpíada Canguru Matemática  2021
